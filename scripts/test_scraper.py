...
async def test_cinode():
    """Test Cinode Market scraper."""
    print("\n" + "="*60)
    print("Testing Cinode Market Scraper")
    print("="*60)
    
    async with CinodeScraper() as scraper:
        # Override max_pages for testing
        scraper.max_pages = 1  # Only scrape first page for testing
        ...
        try:
            ...
            if jobs:
                ...
                # Save to file for inspection
                output_file = f"scraped_jobs_cinode_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(
                        [job.dict() for job in jobs],
                        f,
...
async def test_scraper_connectivity():
    ...
    sites = [
        ...
        ("Cinode Market", "https://cinode.market"),
...
async def main():
    ...
    parser.add_argument(
        "--scraper",
        choices=["brainville", "cinode", "all", "connectivity"],
